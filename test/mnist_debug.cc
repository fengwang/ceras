...
int main()
{
    ...
    {
        std::ofstream ofs{ "./debug/w1.txt" };
        ofs << w1.data() << std::endl;
        ofs.close();
    }
    ...
    {
        std::ofstream ofs{ "./debug/w2.txt" };
        ofs << w2.data() << std::endl;
        ofs.close();
    }
    ...
    for ( auto e : range( epoch ) )
    {
        ...
        for ( auto i : range( iteration_per_epoch ) )
        {
            ...
            std::cout << "Loss at epoch " << e << " index: " << (i+1)*batch_size << ":\t" << current_error[0] << std::endl;
