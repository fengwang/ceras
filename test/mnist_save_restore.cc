...
int main()
{
    ...
    s.restore( "./test/mnist.session" );
    {
        ...
        for ( auto i = 0UL; i != testing_iterations; ++i )
        {
            ...
            {
                errors += 1;
                //std::cout << "[RESTORED SESSION]: Prediction error at " << i << ": predicted " << predicted_number << ", but the ground_truth is " << ground_truth << std::endl;
            }

        }

        float const err = 1.0 * errors / 10000;
        std::cout << "[RESTORED SESSION]: Prediction error on the testing set is " << err << std::endl;
    }
