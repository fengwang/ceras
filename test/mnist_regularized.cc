...
int main()
{
    ...
    // 1st layer
    auto w1 = variable{ randn<float>( {28*28, 256}, 0.0, 10.0/(28.0*16.0) ), l1_reg, l2_reg };
    auto b1 = variable{ zeros<float>( { 256, } ), l1_reg, l2_reg };
    auto l1 = relu( input * w1 + b1 );

    // 2nd layer
    auto w2 = variable{ randn<float>( {256, 128}, 0.0, 3.14/(16.0*11.2 )), l1_reg, l2_reg };
    auto b2 = variable{ zeros<float>( { 128, } ), l1_reg, l2_reg };
    auto l2 = sigmoid( l1 * w2 + b2 );

    // 3rd layer
    auto w3 = variable{ randn<float>( {128, 10}, 0.0, 1.0/35.8 ), l1_reg, l2_reg };
    auto b3 = variable{ zeros<float>( { 10, } ), l1_reg, l2_reg };
