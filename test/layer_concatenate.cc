...
int main()
{
    ...
    auto a = ceras::variable{ random<float>( {2, 3} ) };
    auto b = ceras::variable{ random<float>( {2, 5} ) };
    std::cout << "Testing concatenate with a=\n" << a.data() << std::endl;
    std::cout << "Testing concatenate with b=\n" << b.data() << std::endl;

    auto la = concatenate()(a, b);
